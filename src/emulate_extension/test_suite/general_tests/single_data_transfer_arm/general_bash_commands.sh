...
#gdb scripts
for i in *.s; do sed "s/a.out/$(basename $i .s).actuallyanexecutable/g; s/gdb.log/$i.log/g; s/mem.bin/$(basename $i .s).bin2/g" < ~/gdb_init_script2  > ~/gdb-take-two/arm11_47/src/emulate_extension/test_suite/general_tests/single_data_transfer_arm/$(basename "$i" .s).gdbinit; done
