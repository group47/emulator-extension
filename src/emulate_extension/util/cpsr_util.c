...
bool is_arithmetic(enum OpCode opCode) {
    return opCode == add || opCode == sub || opCode == rsb || opCode == cmp || opCode == adc || opCode == cmn ||
           opCode == sbc || opCode == rsc;
}

bool is_logical(enum OpCode opCode) {
    //return !is_arithmetic(opCode);
    return opCode == and || opCode == eor || opCode == tst || opCode == teq || opCode == orr || opCode == mov ||
