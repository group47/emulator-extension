...
execute_instruction_SP_relative_load_store(const struct SPRelativeLoadStoreInstruction instruction) {

    assert(instruction.filler1001 == 0b1001);

    const uint16_t imm = (uint16_t) (instruction.word * 4);
    ByteAddress address = get_word_from_register(SP_ADDRESS) + imm;
