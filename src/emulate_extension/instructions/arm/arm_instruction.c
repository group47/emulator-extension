    ...
    switch (armInstruction.type) {
        case ARM_BLOCK_DATA_TRANSFER:
            return execute_instruction_block_data_transfer(
                    armInstruction.rawArmInstruction.blockDataTransferInstruction);
        case ARM_BRANCH:
            return execute_instruction_branch(armInstruction.rawArmInstruction.branchInstruction);
        case ARM_BRANCH_AND_EXCHANGE:
            return execute_instruction_branch_and_exchange_instruction(
                    armInstruction.rawArmInstruction.branchAndExchangeInstruction);
        case ARM_COPROCESSOR_DATA_OPERATIONS:
            return execute_instruction_coprocessor_data_operations(
                    armInstruction.rawArmInstruction.coprocessorDataOperationsInstruction);
        case ARM_COPROCESSOR_DATA_TRANSFER:
            return execute_instruction_coprocessor_data_transfers(
                    armInstruction.rawArmInstruction.coprocessorDataTransfersInstruction);
        ...
        case ARM_HALFWORD_AND_SIGNED_DATA_TRANSFER_REGISTER_OFFSET:
            return execute_halfword_and_signed_data_transfer_register_offset(
                    armInstruction.rawArmInstruction.halfWordAndSignedDataTransferRegisterOffset);
        case ARM_HALFWORD_AND_SIGNED_DATA_TRANSFER_IMMEDIATE_OFFSET:
            return execute_halfword_and_signed_data_transfer_immediate_offset(
                    armInstruction.rawArmInstruction.halfWordAndSignedDataTransferImmediateOffset);
        case ARM_MULIPLY:
        ...
        case ARM_SINGLE_DATA_TRANSFER:
            return execute_instruction_single_data_transfer(
                    armInstruction.rawArmInstruction.singleDataTransferInstruction);
        case ARM_SOFTWARE_INTERRUPT:
            return execute_instruction_software_interrupt(
                    armInstruction.rawArmInstruction.softWareInterruptInstruction);
        case ARM_UNDEFINED:
...
struct ArmInstruction ARMfromRaw(union RawArmInstruction raw) {
    ...
    } else if (raw.dataSwapInstruction.filler00010_position27 == 0b00010 &&
               raw.dataSwapInstruction.filler00_position21 == 0b00 &&
               raw.dataSwapInstruction.filler0000_position11 == 0b0000 &&
               raw.dataSwapInstruction.filler1001_position7 == 0b1001) {
        res.type = ARM_SINGLE_DATA_SWAP;
    } else if (raw.branchAndExchangeInstruction.filler0001_0010_1111_1111_1111_0001_position27 ==
               0b000100101111111111110001) {
        res.type = ARM_BRANCH_AND_EXCHANGE;
    } else if (raw.halfWordAndSignedDataTransferRegisterOffset.shouldBe1 == 0b1 &&
               raw.halfWordAndSignedDataTransferRegisterOffset.shouldBe0b000 == 0b000 &&
               raw.halfWordAndSignedDataTransferRegisterOffset.shouldBe0 == 0b0 &&
               raw.halfWordAndSignedDataTransferRegisterOffset.shouldBe00001 == 0b00001) {
        res.type = ARM_HALFWORD_AND_SIGNED_DATA_TRANSFER_REGISTER_OFFSET;
    } else if (raw.halfWordAndSignedDataTransferImmediateOffset.shouldBe0b000 == 0b00 &&
               raw.halfWordAndSignedDataTransferImmediateOffset.filler1position7 == 0b1 &&
               raw.halfWordAndSignedDataTransferImmediateOffset.filler1Position22 == 0b1 &&
               raw.halfWordAndSignedDataTransferImmediateOffset.filler1position4 == 0b1) {
    ...
    } else if (raw.coprocessorDataOperationsInstruction.filler0_position4 == 0b0 &&
               raw.coprocessorDataOperationsInstruction.filler1110_position27 == 0b1110) {
    ...
    } else if (raw.softWareInterruptInstruction.filler1111_position27 == 0b1111) {
        res.type = ARM_SOFTWARE_INTERRUPT;
    } else {
